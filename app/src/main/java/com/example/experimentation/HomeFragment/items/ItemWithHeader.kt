...
class ItemWithHeader(
    val itemsForRecycler: List<RepositoryRemoteItemLatest>? = emptyList(), val viewLatest: Int? = null
    ...
    class ViewHolder(view: View) : FastAdapter.ViewHolder<ItemWithHeader>(view) {
        ...
        override fun bindView(item: ItemWithHeader, payloads: List<Any>) {
            ...
            item.itemsForRecycler?.map { BindingLatestItems(name = it.name, icon = it.image_url, price = it.price, category = it.category ) }
                .let {
                    if (it != null) {
